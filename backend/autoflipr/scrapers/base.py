...
class BaseScraper(ABC):
    ...
    def _jitter_sleep(self, base_seconds: float = 2.0) -> None:
        """Blocking sleep — only safe to call from synchronous (non-async) code."""
        time.sleep(base_seconds * random.uniform(0.5, 1.5))

    async def _jitter_sleep_async(self, base_seconds: float = 2.0) -> None:
        """Non-blocking sleep for use inside async methods."""
        await asyncio.sleep(base_seconds * random.uniform(0.5, 1.5))
