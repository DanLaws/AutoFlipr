...
from autoflipr.celery_app import celery
from autoflipr.db.models import DealScore, Listing, LLMInsight, ListingReport, UserScan, User
from autoflipr.tasks.scan_url import detect_source
...
def _check_and_deduct(db: Session, user_id: int, plan: str) -> None:
    """Enforce monthly scan limit atomically. Raises 429 if exceeded."""
    ...
    # Reset counter if we've rolled into a new month (non-atomic, but safe: a
    # simultaneous reset just writes the same values twice).
    db.execute(
        update(User)
        .where(User.id == user_id, User.scan_month != month)
        .values(scan_count=0, scan_month=month)
    )
    db.flush()

    # Atomic increment — only succeeds if the user is still under the limit.
    result = db.execute(
        update(User)
        .where(User.id == user_id, User.scan_count < limit)
        .values(scan_count=User.scan_count + 1)
        .returning(User.scan_count)
    ).fetchone()
    db.commit()
...
def submit_scan(request: Request, body: ScanRequest, current_user: CurrentUser, db: DBSession):
    url = body.url.strip()
    ...
    celery.send_task("autoflipr.tasks.scan_url.scan_url_task", args=[scan.id], queue="scan")
...
def scan_history(
    ...
    # Collect listing IDs reported by this user so we can suppress detail
    reported_listing_ids: set[int] = set(
        r[0] for r in db.query(ListingReport.listing_id)
        .filter(ListingReport.user_id == current_user["id"])
        .all()
    )

    results = []
    for scan in scans:
        result = _build_result(scan, db)
        # If the listing was reported by this user or globally hidden, strip
        # the scored detail but keep the scan record visible so history is intact.
        if scan.listing and (
            scan.listing_id in reported_listing_ids
            or scan.listing.globally_hidden
        ):
            result.status = "reported"
            result.score = None
            result.estimated_value_gbp = None
            result.estimated_margin_gbp = None
