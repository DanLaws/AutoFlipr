...
from autoflipr.api.deps import DBSession, AdminUser, CurrentUser
from autoflipr.db.models import Listing, DealScore, LLMInsight, MOTHistory, ScrapeRun, ListingReport, ReportType
...
def pipeline_stats(db: DBSession, _user: AdminUser) -> PipelineStats:
    counts = (
...
def get_listing(listing_id: int, db: DBSession, _user: AdminUser) -> ListingDetail:
...
class ReportRequest(BaseModel):
    report_type: ReportType = ReportType.SCAM
...
def recent_scrape_runs(db: DBSession, _user: AdminUser, limit: int = 20) -> list[ScrapeRunSummary]:
