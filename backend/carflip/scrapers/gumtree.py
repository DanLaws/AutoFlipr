...
class GumtreeScraper(BaseScraper):
    ...
    async def run(self, search_params: dict) -> list[RawListing]:
        ...
        async with async_playwright() as p:
            ...
            # ---- Fetch each listing (new page per listing, same browser) ----
            results: list[RawListing] = []
            consent_dismissed = False

            for numeric_id, path in all_pairs.items():
    ...
    # Keep these for BaseScraper ABC compliance — not used when run() is overridden
    ...
    async def fetch_listing(self, listing_id: str) -> Optional[RawListing]:  # type: ignore[override]
        raise NotImplementedError("Use run() directly")
