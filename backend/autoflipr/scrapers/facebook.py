...
from autoflipr.scrapers.base import BaseScraper, RawListing, USER_AGENTS
...
class FacebookLoginWallError(RuntimeError):
    """Raised when the Facebook scraper hits a login wall (cookies expired)."""
...
async def _dismiss_fb_dialogs(page: Page) -> None:
    ...
    Raises FacebookLoginWallError if a login wall is detected (cookies expired).
    """
    # Cookie consent
    for selector in [
    ...
    # Login wall — cookies have expired; surface as a typed error
    for selector in ['[data-testid="royal_login_form"]', 'form[action*="login"]']:
        try:
            el = page.locator(selector)
            if await el.count() > 0:
                raise FacebookLoginWallError("Login wall detected — Facebook cookies have expired")
